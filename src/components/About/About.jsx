...
const About = ({ data }) => {
  ...
  {contactInfo.map((element, index) => {
    const isEmail = element.icon === 'envelope';
    const isMap = element.icon === 'map';

    return (
      <div className={isEmail ? "col-sm-7" : isMap ? "col-sm-5" : "col-sm-6"} key={index}>
        <p>
          <Icon icon={`bi:${element.icon}`} />{' '}
          <span>
            {element.icon === 'phone' ? (
              <a href={`tel:${element.data}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                {element.data}
              </a>
            ) : isEmail ? (
              <a href={`mailto:${element.data}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                {element.data}
              </a>
            ) : (
              element.data
